...
#include <linux/proc_fs.h>/** ZTE_MODIFY yuehongliang added for get touchscreen information, yuehongliang0006 */
#include <mach/hw_ver.h> /** ZTE_MODIFY yuehongliang added for sliding on touchscreen restore sensitivity, yuehongliang0008 */
...
#define GET_TOUCHSCREEN_INFORMATION/** ZTE_MODIFY yuehongliang added for get touchscreen infomation, yuehongliang0006 */
...
int read_mxtouch_nt_t_num(struct mxt_data *mxt, uint16_t *tch, uint16_t *atch)/** ZTE_MODIFY yuehongliang added check calibration for 10" touchscreen, yuehongliang0005 */
{
        ...
        if((data_buffer[0] == 0xF3) && (data_buffer[1] == 0x00))
        {
                ...
                x_line_limit = 16 + 2; /** ZTE_MODIFY yuehongliang modified for channels count, yuehongliang0005 */
                if(x_line_limit > 20)
                {
...
{
            uint16_t tch_ch = 0, atch_ch = 0;/** ZTE_MODIFY yuehongliang added check calibration for 10" touchscreen, yuehongliang0005 */
            int error = 0;
...
{
        uint8_t ATCH_NOR[4] = {255, 1, 0, 0};//ZTE_MODIFY the ts do not calibrate during using
        uint8_t objectT9 = 0;
        uint8_t objectT22 = 0;
        int error;
        ...
        else
        {
                for (loop_i = 0; loop_i < mxt->numtouch; loop_i++) 
                {
                        if (mxt->finger_pressed & BIT(loop_i)) 
                        {
                                ...
                                input_report_abs(mxt->input, ABS_MT_TRACKING_ID, loop_i); /** ZTE_MODIFY wangweiping, added for we must report touch number, wangweiping0017 */
                                input_mt_sync(mxt->input); 
        ...
        switch (object) {
                case MXT_GEN_COMMANDPROCESSOR_T6:
                        ...
                        if (status & MXT_MSGB_T6_CAL) 
                        {
                                ...
                                check_chip2_0_calibration(mxt);  //ZTE_MODIFY atmel 224 ts need calibration when receive T6 message
                                mxt->pre_data[0] = 0;
                                mxt_clean_touch_state(mxt);
                ...
                switch(object_type){
                        ...
                        case MXT_PROCI_GRIPSUPPRESSION_T40:
                                error = mxt_write_block(mxt->client, 
                                                        ...
                                                        pdata->config_T40);/** ZTE_MODIFY yuehongliang modify for bending issues and charger noise suppression, yuehongliang0007 */
                                break;
                        case MXT_PROCI_PALMSUPPRESSION_T41:
                                error = mxt_write_block(mxt->client, 
                                                        get_object_address(mxt, MXT_PROCI_PALMSUPPRESSION_T41), 
                                                        get_object_size(mxt, MXT_PROCI_PALMSUPPRESSION_T41), 
                                                        pdata->config_T41);/** ZTE_MODIFY yuehongliang modify for bending issues and charger noise suppression, yuehongliang0007 */
                                break;
                        case MXT_SPT_DIGITIZER_T43:
                                error = mxt_write_block(mxt->client, 
                                                        get_object_address(mxt, MXT_SPT_DIGITIZER_T43), 
                                                        get_object_size(mxt, MXT_SPT_DIGITIZER_T43), 
                                                        pdata->config_T43);/** ZTE_MODIFY yuehongliang modify for bending issues and charger noise suppression, yuehongliang0007 */
                                break;
					...
					int count, int *eof, void *data)
{
        *eof = 1;
        ...
        return sprintf(page, "   name : Atmel\n" /** ZTE_MODIFY yuehongliang modified for get touchscreen infomation */
                "   i2c address : 0x%x\n"
                "   IC type : 2000 series\n"
			       ...
			       const struct i2c_device_id *id)
{
        ...
        mxt->pre_data[0] = 0;  /** ZTE_MODIFY wangweiping, added for calibration, wangweiping0017 */
