...
//zhaoyang196673 add for mpu3050 gyro begin
#include "mlos.h"
#include "log.h"
//zhaoyang196673 add for mpu3050 gyro end

//zhaoyang196673 add for mpu3050 gyro begin
#include <linux/delay.h>
#define MSM_GSBI12_QUP_I2C_BUS_ID 5
#define MPU_WO_AM_I	0x0
#define MPU_DEV_ID_MASK	0x7e
#define MPU_DEV_ID	0x68
#define LIS3DH_WHO_AM_I 0x0F
#define LIS3DH_ID 0x33
#define MMA8452_WHO_AM_I 0x0D
#define MMA8452_ID 0x2A
#define MMA8452_I2C_SLAVE_ADDR	0x1D
#define LIS3DH_I2C_SLAVE_ADDR	0x19
#define KXTIK_I2C_SLAVE_ADDR	0x0F
#define KXTIK_WHO_AM_I 0x0F
#define KXTIK_1001_ID 0x04
#define KXTIK_1004_ID 0x05
...
static struct mpu_private_data *this_mpu;
                           ...
                           struct mpu_read_write __user *usr_msg)
{
    ...
    switch (cmd) {
        ...
        case MPU_WRITE:
            ...
            break;
        case MPU_I2C_READ:
            printk("mpu_handle_mlsl :read addr=0x%x",addr);
            retval = MLSLSerialRead(sl_handle, addr,
                                    msg.address, msg.length, msg.data);
            break;
        case MPU_I2C_WRITE:
            printk("mpu_handle_mlsl :write addr=0x%x",addr);
            retval = MLSLSerialWrite(sl_handle, addr,
                                     msg.length, msg.data);
            break;
        case MPU_READ_MEM:
                      ...
                      unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case MPU_I2C_READ:
        case MPU_I2C_WRITE:
            retval = mpu_handle_mlsl(client->adapter, mldl_cfg->pdata->accel.address, cmd,
                                     (struct mpu_read_write __user *) arg);
            break;
...
static int mpu3050_disable(void)
{
    struct mldl_cfg *mldl_cfg = &this_mpu->mldl_cfg;
    struct i2c_adapter *accel_adapter;
    struct i2c_adapter *compass_adapter;
    struct i2c_adapter *pressure_adapter;
    ...
    mutex_lock(&this_mpu->mutex);
    if (!mldl_cfg->ignore_system_suspend) {
        printk("@@@@@@@@@@-----zhaoyang mpu3050 has been disabled -------------\n");
        (void) mpu3050_suspend(mldl_cfg, this_client->adapter,
                               accel_adapter, compass_adapter,
                               pressure_adapter,
                               TRUE, TRUE, TRUE, TRUE);
    } else {
        printk("@@@@@@@@@@-----zhaoyang mpu3050 is disable now-------------\n");
    }
    mutex_unlock(&this_mpu->mutex);
    return 0;
}

static int mpu3050_enable(void)
{
    struct mldl_cfg *mldl_cfg = &this_mpu->mldl_cfg;
    struct i2c_adapter *accel_adapter;
    struct i2c_adapter *compass_adapter;
    ...
    mutex_lock(&this_mpu->mutex);
    if (this_mpu->pid && !mldl_cfg->ignore_system_suspend) {
        unsigned long sensors = mldl_cfg->requested_sensors;
        (void) mpu3050_resume(mldl_cfg, this_client->adapter,
                              accel_adapter,
                              compass_adapter,
                              ...
                              sensors & ML_THREE_AXIS_COMPASS,
                              sensors & ML_THREE_AXIS_PRESSURE);
        printk("@@@@@@@@@@-----zhaoyang mpu3050 is enable now-------------\n");
    } else {
        printk("@@@@@@@@@@-----zhaoyang mpu3050 has been enabled -------------\n");
    }
    mutex_unlock(&this_mpu->mutex);
    return 0;
}

static ssize_t mpu3050_store(struct kobject *kobj,
...
extern ssize_t attiny44a_store(struct kobject *kobj,
                               struct kobj_attribute *attr,
                               const char *buf, size_t n);
sensors_attr(attiny44a);
#endif

sensors_attr(mpu3050);
...
static struct kobject *sensors_kobj;

static int __init sensors_sys_init(void)
{
    sensors_kobj = kobject_create_and_add("sensors", NULL);
    if (!sensors_kobj)
        return -ENOMEM;
    return sysfs_create_group(sensors_kobj, &attr_group);
}
...
extern int mpu3050_setup_irq_gpio(void);
                  ...
                  const struct i2c_device_id *devid)
{
    ...
    printk(KERN_INFO "check MPU3050 chip ID\n");

    result = i2c_smbus_read_byte_data(client, MPU_WO_AM_I);
    if (MPU_DEV_ID != (result & MPU_DEV_ID_MASK)) {  //compare the address value
        dev_err(&client->dev,"read chip ID 0x%x is not equal to MPU3050_ID 0x%x!\n", (result & MPU_DEV_ID_MASK), MPU_DEV_ID);
        printk(KERN_INFO "read MPU3050 chip ID failed\n");
        //return (-ENODEV);   //should return error, but we found sometime if it's wrong,but devices is still work
    } else {
        printk(KERN_INFO "@@@@@@@@@zhaoyang196673 read MPU3050 chip ID OK!\n");
    }
    //zhaoyang196673 add for mpu3050 gyro end
    ...
    if (!pdata) {
        ...
        /////////////////////////////////////////////
        printk("$$$$$ MPU3050 detect acc type now -----\n");
        accel_adapter = i2c_get_adapter(MSM_GSBI12_QUP_I2C_BUS_ID);

        printk("$$$$$ MPU3050 is detecting KXTIK acc -----\n");
        result = MLSLSerialRead(accel_adapter, KXTIK_I2C_SLAVE_ADDR, KXTIK_WHO_AM_I, 1,  &product_id);

        if((product_id == KXTIK_1004_ID) || (product_id == KXTIK_1001_ID)) {
            printk("$$$$$ MPU3050 have detected KXTIK acc -----\n");
            pdata->accel.get_slave_descr = kxtf9_get_slave_descr;
            pdata->accel.address = KXTIK_I2C_SLAVE_ADDR;
            printk("accelerometer detect SUCCESS\n");
            goto acc_found;
        } else {
            printk("$$$$$ MPU3050 detect KXTIK acc FAILED !!!\n");
            printk("@@@@@@@@@@----- KXTIK product_id = 0x%x-------------\n",product_id);
        }

        printk("$$$$$ MPU3050 is detecting MMA8452 acc -----\n");
        result = MLSLSerialRead(accel_adapter, MMA8452_I2C_SLAVE_ADDR, MMA8452_WHO_AM_I, 1, &product_id);
        if(product_id == MMA8452_ID) {
            printk("$$$$$ MPU3050 have detected MMA8452 acc -----\n");
            pdata->accel.get_slave_descr = mma845x_get_slave_descr;
            pdata->accel.address = MMA8452_I2C_SLAVE_ADDR;
            printk("accelerometer detect SUCCESS\n");
            goto acc_found;
        } else
            printk("$$$$$ MPU3050 detect MMA8452 acc FAILED !!!\n");

        printk("$$$$$ MPU3050 is detecting LIS3DH acc -----\n");
        result = MLSLSerialRead(accel_adapter, LIS3DH_I2C_SLAVE_ADDR, LIS3DH_WHO_AM_I, 1,  &product_id);
        if(product_id == LIS3DH_ID) {
            printk("$$$$$ MPU3050 have detected LIS3DH acc -----\n");
            pdata->accel.get_slave_descr = lis3dh_get_slave_descr;
            pdata->accel.address = LIS3DH_I2C_SLAVE_ADDR;
            printk("accelerometer detect SUCCESS\n");
        } else {
            printk("$$$$$ MPU3050 detect KXTIK & LIS3DH & MMA8452 acc both FAILED, return error !!!\n");
            return ENODEV;
        }
    ...
    defined(CONFIG_MPU_SENSORS_MPU6000_MODULE)
        ...
        if (pdata->accel.get_slave_descr) {
            ...
            if (pdata->accel.irq > 0) {
                ...
                printk("WARNING: Accel irq not assigned\n");
            }
        } else {
            printk( "%s: No Accel Present\n", MPU_NAME);
        }
        ...
        if (pdata->compass.get_slave_descr) {
            ...
            printk("compass detect SUCCESS\n");
            if (pdata->compass.irq > 0) {
                printk("Installing Compass irq using %d\n",
                       pdata->compass.irq);
                ...
                printk("WARNING: Compass irq not assigned\n");
            }
        } else {
            printk("%s: No Compass Present\n", MPU_NAME);
        }
        ...
        if (pdata->pressure.get_slave_descr) {
            ...
            if (pdata->pressure.irq > 0) {
                printk("Installing Pressure irq using %d\n",
                       pdata->pressure.irq);
                ...
                printk("WARNING: Pressure irq not assigned\n");
            }
        } else {
            printk("%s: No Pressure Present\n", MPU_NAME);
        }
    }
...
#endif
    printk("gyroscope detect SUCCESS\n");

    this_mpu =	(struct mpu_private_data *) i2c_get_clientdata(client);
    sensors_sys_init();
