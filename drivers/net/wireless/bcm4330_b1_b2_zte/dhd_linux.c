...
----------    ------------     -----------      --------------------------      
2011/07/22    longchunyan0002   longchunyan      repaired for bcm4330 b1&b2
2011/09/30    longchunyan0003   longchunyan      reapir for wifi sleeping
2011/10/01    longchunyan0004   longchunyan      add board information
2012/01/16    chengjia0052      chengjia         fix bug to avoid kernel panic
2012/01/17    chengjia0054      chengjia         fix bug to avoid timer download
2012/01/31    xxxxxxx          dupucun10130370  close the dhd driver polling the FW debug log, it can reduce some loading for FW. 
2012/04/25    chengjia0072      chengjia              download different nvram
===========================================================================*/
...
char firmware_path[MOD_PARAM_PATHLEN] = "";
char nvram_path[MOD_PARAM_PATHLEN] = "persist/zte/wifi/bcm4330.txt";
...
static void dhd_watchdog(ulong data)
{
	...
	if (printk_ratelimit()) {
		printk("chengjiatest: dhd_watchdog timer avoid panic!");
	}
#if 0
	/* Call the bus module watchdog */
	dhd_bus_watchdog(&dhd->pub);
...
extern  void wl_iw_restart(struct net_device *dev);
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
    ...
    if((buflen > 3) && (!strncmp((char*)buf, "ver1", 4)))
	{
		printk("Tony====  OK right!!!\n");
		wl_iw_restart(net);	
        goto done;
	}
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (firmware_path != NULL) 
	{
        ...
        {
            if((HW_VERSION_V55_B == hw_ver) || (HW_VERSION_V55_C == hw_ver)) 
            {
                if (chip_is_b1)
	                strcpy(fw_path, v55_new_fw_4330_b1_path);
	            else
	        	    strcpy(fw_path, v55_new_fw_4330_b2_path);
             
             printk("lcy test:firmware is V55_B or V55_C\n");

            }
            else
            {
                if (chip_is_b1)
	        	     strcpy(fw_path, fw_4330_b1_path);
	            else
	        	     strcpy(fw_path, fw_4330_b2_path);
                
                printk("lcy test:firmware is general\n");
            }
        ...
        {
            strcpy(fw_path, firmware_path);
            printk("lcy test:firmware is testing\n");
	...
	if ((nvram_path != NULL) && (nvram_path[0] != '\0'))
	{
        if(HW_VERSION_V55_A == hw_ver)  
        {
            if (chip_is_b1)
	        	strcpy(nv_path, v55_old_nv_4330_b1_path);
	        else
	        	strcpy(nv_path, v55_old_nv_4330_b2_path);
            
            printk("lcy test:nvram is V55_A\n");
            
        }
#if 0
	 else if(HW_VERSION_V66_C == hw_ver)  
        {

       	strcpy(nv_path, v66_old_nv_4330_b2_path);
            
              printk("chengjiatest:nvram is V66_C\n");
        ...
        {
            if (chip_is_b1)
	        	strcpy(nv_path, nv_4330_b1_path);
	        else
	        	strcpy(nv_path, nv_4330_b2_path);
            
            printk("lcy test:nvram is general\n");
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	if  ((dhd->pub.busstate == DHD_BUS_DOWN) &&
		...
		printk("load firmware from %s,nvram from %s\n", fw_path,nv_path);
...
extern void enable_sdcc4_detect(void);
...
dhd_module_init(void)
{
    ...
    enable_sdcc4_detect();/////chengjiatest added for mmc scan
...
dhd_dev_reset(struct net_device *dev, uint8 flag)
{
    ...
    if (ret) 
    {
        printk("lcy test:dhd_dev_reset error: %d\n", ret);
        return ret;
    }
    printk("lcy test:dhd_dev_reset sucess\n");
    return ret;
    #endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25)) && 1
void dhd_os_start_lock_init(dhd_pub_t *pub)
{
    dhd_info_t *dhd = (dhd_info_t *)(pub->info);
