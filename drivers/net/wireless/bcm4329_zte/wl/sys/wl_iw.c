 ...
 *
 * $Id: wl_iw.c,v 1.51.4.9.2.6.4.142.4.63 2010/12/10 01:33:50 Exp $
 */
...
)
{
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		WL_ERROR(("chengjiatest: %s: call after driver stop : ignored\n", __FUNCTION__));
	}
	return ret;
}
...
WL_ERROR(("chengjiatest: %d     val = %d\n", iw->privacy_invoked, val));
		if (iw->privacy_invoked && !val) {
			WL_ERROR(("%s: %s: 'Privacy invoked' TRUE but clearing wsec, assuming "
			...
			if ((error = dev_wlc_intvar_set(dev, "is_WPS_enrollee", FALSE))) {
				WL_ERROR(("Failed to clear iovar is_WPS_enrollee, error=%d\n", error));
				return error;
			}
		}
	...
	case IW_AUTH_WPA_ENABLED:
		if (paramval == 0) {
			...
			WL_ERROR(("Failed to set 'wpa_auth'iovar %d\n", error));
			return error;
		}
...
static int set_ap_cfg(struct net_device *dev, struct ap_profile *ap)/////////////chengjiatest ap
{
	int updown = 0;
	int channel = 0;
