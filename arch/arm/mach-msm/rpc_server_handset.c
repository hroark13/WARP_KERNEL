...
#include <linux/pmic8058-othc.h> //by zrlean
#include <linux/timer.h>//by zrlean
#include <mach/vreg.h> //zrlean
...
static struct hs_key_t hsi_key;
static hs_type_t hs_type = HS_NO_HEADSET;

static void input_report_hph_event(unsigned long data);
...
struct msm_handset {
	...
	#ifdef ZTE_FEATURE_HEADPHONE
	struct timer_list hph_timer; /* Timer to send headset insert event */
	#endif
...
static void input_report_hph_event(unsigned long data)
{
	uint32_t key_code = ((struct hs_key_t*)data)->key_code;

	if (HS_STEREO_HEADSET_K == key_code) { /* Headset is inserted */
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	if (key_parm == HS_REL_K)
		key_code = key_parm;
	printk(KERN_ERR"[tagL] key = %d key_code = %d\n",key,key_code);
	switch (key) {
	case KEY_POWER:
	case KEY_SLEEP:
//	case KEY_END:
...
case KEY_WAKEUP:
	...
	case SW_HEADPHONE_INSERT_W_MIC:
		...
		if (key_parm == HS_REL_K) { /* Headset is removed, del timer and report immediately */
			...
			hsi_key.key_code = key_code;
			hsi_key.key_parm = key_parm;

			init_timer(&hs->hph_timer);
			hs->hph_timer.data = (unsigned long)&hsi_key;
			hs->hph_timer.function = input_report_hph_event;
			hs->hph_timer.expires = jiffies + (910 * HZ)/1000; /* 910ms */
			add_timer(&hs->hph_timer);
		}
		#else 
