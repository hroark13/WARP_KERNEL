...
Geschichte:
Wenn               Wer          Was                                                                        Tag
2011-02-19    wangzy     create for bh1721 ambient light sensor     
...
static int bh1721_config_rst_gpio(int rst_gpio);
...
static int __init als_init(void) {
	...
	/*DVI enable at least 1 us after VCC supply*/
	als_datap->rst_gpio = ALS_RST_GPIO;
    	ret = gpio_request(als_datap->rst_gpio, "gpio_sensor");
    	if (ret) {
        	printk(ALS_TAG "gpio_request failed in als_init(),%d\n", ret);
		return (ret);	
    	}
	else
    	{
		ret=bh1721_config_rst_gpio(als_datap->rst_gpio);
		if(ret){
			printk(KERN_ERR "BH1721: reset gpio config failed in als_init(),%d\n",ret);
			return (ret);
		}
	}
...
/* ----------------------*
* config gpio for als reset        *
*-----------------------*/
static int bh1721_config_rst_gpio(int rst_gpio)
{
    int rc=0; 
    uint32_t  gpio_config_data = GPIO_CFG(rst_gpio,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA);
...
static int __devexit als_remove(struct i2c_client *client) {
	int ret = 0;
	if(als_datap->rst_gpio)
		gpio_free(als_datap->rst_gpio);
