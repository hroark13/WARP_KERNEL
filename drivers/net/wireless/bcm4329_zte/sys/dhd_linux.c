...
----------    ------------     -----------      --------------------------      
2011/04/27    chengj0010          chengjia            modify for wifi mmc poll change
2011/07/08    longchunyan0002     longchunyan         repaired for nvram path
2011/07/12    longchunyan0003     longchunyan         modify for notifing standard
===========================================================================*/
...
static int wifi_probe(struct platform_device *pdev)
{
	...
	printk("chengjiatest: platform driver wifi power!!!!!!!!!!!!!!!!!!!!!!1");
 ...
 */
char firmware_path[MOD_PARAM_PATHLEN] = "/system/etc/wifi/bcm4329.bin";
...
_dhd_set_multicast_list(dhd_info_t *dhd, int ifidx)
{
	...
	struct netdev_hw_addr *ha; // hcc 110309
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35)
	cnt = netdev_mc_count(dev); // hcc 110309
#else
	cnt = dev->mc_count;
#endif
	netif_addr_unlock_bh(dev);  // hcc 110309
...
dhd_module_init(void)
{
	...
	DHD_ERROR(("%s:   nvram_path = %s  Enter\n", __FUNCTION__, nvram_path));
	DHD_ERROR(("%s:   firmware_path = %s  Enter\n", __FUNCTION__, firmware_path));
	...
	dhd_customer_gpio_wlan_ctrl(WLAN_POWER_ON);//打开高通的wifi相关电源

#if defined(CUSTOMER_HW2) && defined(CONFIG_WIFI_CONTROL_FUNC)
	sema_init(&wifi_control_sem, 0);

	error = wifi_add_dev();///这个没有用到，platform bus??
	if (error) {
