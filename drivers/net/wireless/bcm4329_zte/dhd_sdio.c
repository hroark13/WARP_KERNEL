...
#endif
/////////////////////chengjiatest for bus down error
#if 0
...
dhd_bus_register(void)
{
	...
	return bcmsdh_register(&dhd_sdio);///////////attach and unattach
}
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	if(bus->fw_path != NULL) {
		DHD_ERROR(("chengjiatest: fw path right  %s\n", bus->fw_path));
	}
	else {
		DHD_ERROR(("chengjiatest: fw path error\n"));
	}
	/* External image takes precedence if specified */
	if ((bus->fw_path != NULL) && (bus->fw_path[0] != '\0')) {
		DHD_ERROR(("chengjiatest: fw path in\n"));
		if (dhdsdio_download_code_file(bus, bus->fw_path)) {
			DHD_ERROR(("chengjiatest: %s: dongle image file download failed\n", __FUNCTION__));
#ifdef BCMEMBEDIMAGE
			embed = TRUE;
#else
			goto err;
...
#ifdef BCMEMBEDIMAGE
DHD_ERROR(("chengjiatest: embed = %d \n", embed));
	if (embed) {
