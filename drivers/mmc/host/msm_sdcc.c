...
{
	//slot 2,4 is emmc sd slot in 8255
	if (id == 2 || id == 4) {
		return 1;
	}
	return 0;
...
//end
	...
	pr_info("%s: polling timer enable = %d\n",
		mmc_hostname(mmc), host->enable_polling_timer);
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	printk(KERN_ERR"jzq:%s: %d msmsdcc_runtime_suspend begin\n", mmc_hostname(host->mmc), __LINE__); 
...
/*
		...
		#ifdef CONFIG_BCM_WIFI
		if (!mmc->card || mmc->card->type != MMC_TYPE_SDIO || host->pdev_id != 3)//shaohua add for wifi suspend
...
printk(KERN_ERR"jzq:%s: %d msmsdcc_runtime_suspend end\n", mmc_hostname(host->mmc), __LINE__); 	
	return rc;
}
...
msmsdcc_runtime_resume(struct device *dev)
{
	...
	printk(KERN_ERR"jzq:%s: %d msmsdcc_runtime_resume begin\n", mmc_hostname(host->mmc), __LINE__); 
	...
	if (mmc) {
      ...
      printk(KERN_ERR"jzq:%s: %d msmsdcc_runtime_resume end\n", mmc_hostname(host->mmc), __LINE__); 
...
static int msmsdcc_pm_suspend(struct device *dev)
{
	...
	printk(KERN_ERR"jzq:%s: %d msmsdcc_pm_suspend begin\n", mmc_hostname(host->mmc), __LINE__);   
	...
	printk(KERN_ERR"jzq:%s: %d msmsdcc_pm_suspend end\n", mmc_hostname(host->mmc), __LINE__);  
...
static int msmsdcc_pm_resume(struct device *dev)
{
	...
	printk(KERN_ERR"jzq:%s: %d msmsdcc_pm_resume begin\n", mmc_hostname(host->mmc), __LINE__);   
	...
	printk(KERN_ERR"jzq:%s: %d msmsdcc_pm_resume end\n", mmc_hostname(host->mmc), __LINE__);  
