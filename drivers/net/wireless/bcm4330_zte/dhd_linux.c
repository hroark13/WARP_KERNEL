...
----------    ------------     -----------      --------------------------      
2011/06/03    longchunyan0001   longchunyan      repair dhd_dev_reset()
2011/07/01    longchunyan0002   longchunyan      add hardware board information
2011/07/04    longchunyan0003   longchunyan      repair for wifi sleeping
2011/07/12    longchunyan0004   longchunyan      repair for notifing standard
===========================================================================*/
...
char nvram_path[MOD_PARAM_PATHLEN] = "/persist/zte/wifi/bcm.txt";
...
if (firmware_path != NULL)
	{
     ...
     {
         if(HW_VERSION_V55_B == hw_ver) 
         {
             strcpy((char *)firmware_path,"/system/etc/wifi/bcm4330_v55_new.bin");
             printk("lcy test:firmware is V55_B\n");

         }
         else
         {
            strcpy((char *)firmware_path,"/system/etc/wifi/bcm4330.bin");
	...
	if ((nvram_path != NULL) && (nvram_path[0] != '\0'))
	{
       if(HW_VERSION_V55_A == hw_ver) 
        {
            strcpy((char *)nvram_path,"/persist/zte/wifi/bcm4330.txt");
        ...
        else
        {
            ...
            printk("lcy test:nvram_path is general\n");
        }
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	if  ((dhd->pub.busstate == DHD_BUS_DOWN) &&
		...
		printk("chengjiatest: firmware_path = %s; nvram_path = %s\n", fw_path, nv_path);
...
dhd_dev_reset(struct net_device *dev, uint8 flag)
{
	...
	#else
	if (ret) {
		printk("longchunyan test:dhd_dev_reset error: %d\n", ret);
		return ret;
	}
	printk("longchunyan test:dhd_dev_reset sucess\n");
	return ret;
  #endif
}
