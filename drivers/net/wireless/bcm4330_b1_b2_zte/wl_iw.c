...
----------    ------------     -----------      --------------------------      
2011/07/30    longchunyan0001   longchunyan      repaired for bcm4330 b1&b2
2011/08/08    longchunyan0003   longchunyan      added for enabling softap confingation
2011/10/08    longchunyan0004   longchunyan      repaired for connecting to hidden 802.11n ap
===========================================================================*/
...
#if defined(SOFTAP)
#define WL_SOFTAP(x)
struct net_device *priv_dev;
EXPORT_SYMBOL(priv_dev);
static bool		ap_cfg_running = FALSE;
bool            ap_fw_loaded = FALSE;
EXPORT_SYMBOL(ap_fw_loaded);
...
static int set_ap_cfg_restart(struct net_device *dev, struct ap_profile *ap); 
...
#ifndef AP_ONLY
static int
thr_wait_for_2nd_eth_dev_restart(void *data)
{
	wl_iw_t *iw;
	int ret = 0;
...
//	}
	...
	if (tsk_ctl->terminated) {
		...
		WL_ERROR((" thr_wait_for_2nd_eth_dev_restart !!!\n"));
...
static int
set_ap_cfg_restart(struct net_device *dev, struct ap_profile *ap)
{
	int updown = 0;
	int channel = 0;
	...
	if (ap_cfg_running == FALSE) {
		...
		ampdu=0;
		WL_ERROR(("%s ===========set ampdu\n", __FUNCTION__));
		if ((res = dev_wlc_intvar_set(dev, "ampdu", ampdu))) {
			WL_ERROR(("%s fail to set ampdu\n", __FUNCTION__));
			goto fail;
		}
#ifdef AP_ONLY
	...
	if ((res = dev_wlc_ioctl(dev, WLC_SET_VAR, buf, iolen)) != 0) {
		...
		frameburst = 1;
		WL_ERROR(("%s +++++++++++++WLC_SET_FAKEFRAG testing.... \n", __FUNCTION__));
		if ((res = dev_wlc_ioctl(dev, WLC_SET_FAKEFRAG, &frameburst, sizeof(frameburst))) < 0) {
			WL_ERROR(("%s fail to set frameburst \n", __FUNCTION__));
			goto fail;
		}

	if (ap_cfg_running == FALSE) {
		
		PROC_START(thr_wait_for_2nd_eth_dev_restart, dev, &ap_eth_ctl, 0);
	} else {
...
set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	if ((res = dev_wlc_ioctl(dev, WLC_SET_VAR, buf, iolen)) != 0) {
        ...
        frameburst = 1;
		WL_ERROR(("%s +++++++++++++WLC_SET_FAKEFRAG test2.... \n", __FUNCTION__));
		if ((res = dev_wlc_ioctl(dev, WLC_SET_FAKEFRAG, &frameburst, sizeof(frameburst))) < 0) {
			WL_ERROR(("%s fail to set frameburst2 \n", __FUNCTION__));
			goto fail;
		}
	...
	char *ext)
{
	...
	if ((wrqu->data.length > 4) && (wrqu->data.length < sizeof(extra))) {
		...
		if  (strstr(fwstr, "apsta") != NULL) {
              ...
              {
                if (chip_is_b1)
	                strcpy(fw_path, "system/etc/wifi/apsta_bcm4330_v55_b1_new.bin");
	            else
	        	    strcpy(fw_path, "system/etc/wifi/apsta_bcm4330_v55_b2_new.bin");
              }
              else if (chip_is_b1)
	        	strcpy(fwstr, "system/etc/wifi/apsta_bcm4330_b1.bin");
              printk("lcy test:fwstr is %s\n",(char *)fwstr);
			  ap_fw_loaded = TRUE;
		} else {
...
)
{
	...
	if (dwrq->length && extra) {
		if (strnicmp(extra, "START", strlen("START")) == 0) {
            ...
            if (ret)
                printk("!!!!!!!!!!!!DRIVER START ERROR,ret is %d!!!!!!!!!!!!!\n",ret);
            #endif
...
#endif 
#ifdef WL_PROTECT
extern void dhd_os_start_lock_init(dhd_pub_t *pub);
static void 
...
static void wl_iw_ap_restart(void)
{
        ...
        set_ap_cfg_restart(priv_dev, &my_ap);
...
void wl_iw_restart(struct net_device *dev)
{
    net_os_wake_lock(dev);
    printk("lcytest:before restarting,lock\n");
	wl_iw_sta_restart(dev);
	if ( ap_cfg_running ) 
		wl_iw_ap_restart();	
  
	/* reset the hang counter */
	net_os_clear_hang_message(dev); 
    net_os_wake_unlock(dev);
    printk("lcytest:after restarting,unlock\n");
}
EXPORT_SYMBOL(wl_iw_restart);
